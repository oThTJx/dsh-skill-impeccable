 ...
 *   pnpm --filter @firefly0621/dsh-skill-impeccable pack
 */
...
if (missing.length > 0) {
  console.error(
    '@firefly0621/dsh-skill-impeccable prepack: missing required files:\n'
      + missing.map((rel) => `  - ${rel}`).join('\n')
      + '\nRun: pnpm run build:lib:host'
      + '\nThen: pnpm --filter @firefly0621/dsh-skill-impeccable pack',
  )
  process.exit(1)
}
...
if (srcMtime > libMtime) {
  console.error(
    '@firefly0621/dsh-skill-impeccable prepack: src/index.ts is newer than lib/index.js.\n'
