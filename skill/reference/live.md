...
2. Open the app URL that serves `pageFile` (infer from `package.json`, docs, terminal output, or an open tab). Never use `serverPort`; it's the helper, not the app. Open the app URL with the available browser tool; if the URL is uncertain, ask the user once.
...
Harness policy:
- **DeepSeek Harness**: run the poll as a background job (`run_in_background: true`), read results with `job_output`, and never end the turn while a poll is outstanding.
